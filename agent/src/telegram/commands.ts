...
export async function handleHelp(ctx: Context, _deps: CommandDeps): Promise<void> {
  const msg = [
    '*AeroFyta Commands*',
    '',
    '/start — Welcome message explaining AeroFyta',
    '/tip `@user amount chain` — Send a tip',
    '  Example: `/tip @sarah_creates 2.5 polygon`',
    '/balance — Show wallet balances across all 9 chains',
    '/status — Agent status (running cycle, tips sent, uptime)',
    '/wallets — Show all 9 wallet addresses',
    '/history — Show last 10 tips',
    '/gas — Show gas prices across chains',
    '/reasoning — Show last agent reasoning chain',
