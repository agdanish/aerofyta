...
export function DcaPanel() {
  ...
  const create = async () => {
    try {
      ...
      await load();
    } catch { /* create failed — form stays open */ }
  };

  const pause = async (id: string) => { try { await api.dcaPause(id); await load(); } catch { /* pause failed — no UI change */ } };
  const resume = async (id: string) => { try { await api.dcaResume(id); await load(); } catch { /* resume failed — no UI change */ } };
  const cancel = async (id: string) => { try { await api.dcaCancel(id); await load(); } catch { /* cancel failed — no UI change */ } };
