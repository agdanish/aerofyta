...
export function ProofOfEngagementPanel() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Attestations initializing');
    }
    setLoading(false);
  };
  ...
  return (
    <div className="rounded-xl border border-border bg-surface-1 p-5 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={load} aria-label="Refresh attestations" className="text-xs text-text-muted hover:text-accent transition-colors flex items-center gap-1">
      ...
      {!error && attestations.length > 0 ? (
            ...
            <div key={att.id} className="p-2.5 rounded-lg bg-surface-2 border border-border card-hover animate-list-item-in" style={{ animationDelay: `${i * 50}ms` }}>
              <div className="flex items-center justify-between mb-1.5">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs text-text-primary font-mono">{att.proof.payloadHash.slice(0, 16)}...</span>
                </div>
                <div className="flex items-center gap-1.5">
                  {verifyResult?.id === att.id ? (
                    verifyResult.valid ? (
                      <span className="text-xs text-green-400 flex items-center gap-0.5"><CheckCircle2 className="w-3 h-3" /> Valid</span>
                    ) : (
                      <span className="text-xs text-red-400 flex items-center gap-0.5"><XCircle className="w-3 h-3" /> Invalid</span>
                    )
                  ) : (
                    <button
                      ...
                      className="text-xs px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-400 hover:bg-amber-500/20 transition-colors btn-press disabled:opacity-50"
                    >
        ...
        <div className="text-center py-4">
          ...
          <p className="text-xs text-text-muted">No attestations yet. Tips with engagement data generate PoE proofs.</p>
        </div>
      ) : null}

      <div className="p-2 rounded-lg bg-amber-500/5 border border-amber-500/10 text-xs text-text-muted">
        <Fingerprint className="w-3 h-3 text-amber-400 inline mr-1" />
