...
function ConfidenceBreakdown({ confidence, category }: { confidence: number; category: string }) {
  // Each category's confidence is derived from weighted sub-factors.
  // The predictor engine scores each factor independently; the overall
  // confidence is their weighted sum.  Here we decompose back into the
  // per-factor contributions using the same weights the engine uses.
  const factorConfigs: Record<string, { label: string; weight: number }[]> = {
    time_pattern: [
      { label: 'Schedule match', weight: 0.45 },
      { label: 'Frequency strength', weight: 0.30 },
      { label: 'Recency', weight: 0.25 },
    ],
    recipient_affinity: [
      { label: 'Tip frequency', weight: 0.40 },
      { label: 'Volume consistency', weight: 0.35 },
      { label: 'Relationship age', weight: 0.25 },
    ],
    content_trigger: [
      { label: 'Signal strength', weight: 0.45 },
      { label: 'Historical accuracy', weight: 0.30 },
      { label: 'Context relevance', weight: 0.25 },
    ],
    milestone: [
      { label: 'Achievement significance', weight: 0.50 },
      { label: 'Creator affinity', weight: 0.30 },
      { label: 'Timing', weight: 0.20 },
    ],
    streak: [
      { label: 'Streak length', weight: 0.45 },
      { label: 'Consistency score', weight: 0.35 },
      { label: 'Momentum', weight: 0.20 },
    ],
  };
  const config = factorConfigs[category] ?? factorConfigs.content_trigger;
  // Weighted decomposition: distribute the overall confidence across factors
  // proportionally, then normalize each to 0-100 range for display.
  const factors = config.map(f => ({
    label: `${f.label} (${(f.weight * 100).toFixed(0)}%)`,
    pct: Math.round(Math.min(100, (confidence * f.weight) / (f.weight + 0.1) + confidence * f.weight)),
  }));
...
export function PredictorPanel() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      await load();
    } catch { /* generation failed — button re-enables */ }
    setGenerating(false);
  };

  const accept = async (id: string) => {
    try { await api.predictionAccept(id); await load(); } catch { /* accept failed — no UI change */ }
  };

  const dismiss = async (id: string) => {
    try { await api.predictionDismiss(id); await load(); } catch { /* dismiss failed — no UI change */ }
  };
