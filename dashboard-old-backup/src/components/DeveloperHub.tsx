...
const PATHS: IntegrationPath[] = [
  {
    ...
    title: 'SDK',
    description: 'Import the AeroFyta client into any Node.js or browser app. Send tips, manage wallets, and subscribe to events in 3 lines of code.',
    language: 'typescript',
    ...
    <div className="animated-border shadow-depth">
      <div className="p-4 sm:p-6">
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold">
              <span className="gradient-text-animated">Build on AeroFyta</span>
            </h2>
          </div>
        ...
        <p className="text-xs text-text-secondary mb-4">
          AeroFyta is infrastructure. Four integration paths to embed programmable payments into any app, agent, or workflow.
        </p>
          ...
          {PATHS.map((path, i) => (
            ...
            >
              ...
              <button className="mt-3 w-full flex items-center justify-center gap-1.5 py-2 rounded-lg bg-surface-2 hover:bg-surface-3 border border-border text-xs font-medium text-text-secondary hover:text-text-primary transition-all btn-press group">
                Get Started
                <ArrowRight className="w-3 h-3 transition-transform group-hover:tranneutral-x-0.5" />
              </button>
            </div>
          ))}
        </div>
