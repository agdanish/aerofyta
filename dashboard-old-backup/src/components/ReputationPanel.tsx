    ...
    <div className="rounded-xl border border-neutral-800 bg-neutral-900/60 p-5">
      ...
      <div className="flex gap-1.5 mb-4">
        <div className="relative flex-1">
          <Search className="absolute left-2.5 top-1/2 -tranneutral-y-1/2 w-3 h-3 text-text-muted" />
          <input
            type="text"
            value={searchAddr}
          ...
          />
          {searchAddr && (
            <button onClick={() => { setSearchAddr(''); setSearchResult(null); }} className="absolute right-2 top-1/2 -tranneutral-y-1/2 text-text-muted hover:text-text-primary">
              <X className="w-3 h-3" />
            </button>
          )}
        </div>
      ...
      {searchResult && (
        <div className="p-3 rounded-lg border border-yellow-500/20 bg-yellow-500/5 mb-4 animate-fade-in">
          ...
          <div className="grid grid-cols-3 gap-2 mt-2 text-xs">
            ...
            <div className="text-center">
              <div className="font-bold text-text-primary">{(searchResult.totalReceived ?? 0).toFixed(4)}</div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-1.5 flex items-center gap-2">
                      ...
                      <span className="text-xs text-text-muted whitespace-nowrap">{(creator.score ?? 0).toFixed(0)}</span>
                    </div>
                  </div>
                ...
                {isExpanded && (
                  <div className="px-3 pb-3 border-t border-border pt-2 animate-slide-down">
                    <div className="grid grid-cols-3 gap-3 text-center">
                      ...
                      <div>
                        <div className="text-sm font-bold text-text-primary">{(creator.avgTipAmount ?? 0).toFixed(4)}</div>
                        <div className="text-xs text-text-muted">Avg Amount</div>
                      </div>
                    </div>
                    <div className="mt-2 flex items-center justify-between text-xs text-text-muted">
                      <span>Total: {(creator.totalReceived ?? 0).toFixed(4)} USDT</span>
                      <span>Raw: {(creator.rawScore ?? 0).toFixed(1)}</span>
                    </div>
                    {/* Mini score history */}
                        ...
                        {creator.scoreHistory.slice(-12).map((point, i) => {
                          ...
                          return (
                            <div
                              ...
                              title={`${point.date}: ${(point.score ?? 0).toFixed(0)}`}
                            />
                          );
                        })}
                      </div>
