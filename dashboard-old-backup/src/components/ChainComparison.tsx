...
function ChainCard({ chain, color, recommendation }: {
  ...
  const colorClasses = color === 'blue'
    ...
    : { border: 'border-lime-500/30', bg: 'bg-lime-500/5', accent: 'text-lime-400', badge: 'bg-lime-500/15 border-lime-500/30 text-lime-400', bar: 'bg-lime-500' };
      ...
      {!collapsed && (
        <>
          {loading && !data ? (
            ...
            <p className="text-xs text-text-muted text-center py-4">Chain analytics initializing — connect the agent backend</p>
          )}
        </>
      )}
    </div>
