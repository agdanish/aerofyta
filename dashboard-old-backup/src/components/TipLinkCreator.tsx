...
export function TipLinkCreator() {
  ...
  const handleShareTwitter = (link: TipLink) => {
    ...
    const text = `Send me a tip of ${link.amount} ${tokenLabel} via AeroFyta! Powered by Tether WDK.`;
  ...
  const handleShareWhatsApp = (link: TipLink) => {
    const url = getFullUrl(link);
    const tokenLabel = link.token === 'usdt' ? 'USDT' : 'crypto';
    const text = `Send me a tip of ${link.amount} ${tokenLabel} via AeroFyta! ${url}`;
    ...
    <div className="rounded-xl border border-border bg-surface-1 overflow-hidden">
      ...
      >
        <h2 className="text-base font-semibold text-text-primary flex items-center gap-2">
          ...
          Tip Links
          {tipLinks.length > 0 && (
            <span className="ml-1 px-1.5 py-0.5 rounded-full bg-orange-500/15 border border-orange-500/20 text-xs font-medium text-orange-400">
              {tipLinks.length}
            </span>
          )}
        </h2>
          ...
          <form onSubmit={handleCreate} className="space-y-3">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 rounded-lg bg-surface-2 border border-border text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-orange-500/50 focus:ring-1 focus:ring-orange-500/30 transition-colors font-mono"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-lg bg-surface-2 border border-border text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-orange-500/50 focus:ring-1 focus:ring-orange-500/30 transition-colors"
                />
              </div>
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 rounded-lg bg-surface-2 border border-border text-sm text-text-primary focus:outline-none focus:border-orange-500/50 transition-colors"
                >
            ...
            <button
              ...
              className="w-full py-2.5 rounded-lg bg-orange-600 text-white text-sm font-medium hover:bg-orange-500 disabled:opacity-40 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
            >
              {creating ? (
                <>
