            ...
            <div key={c.id} className="rounded-xl border border-border bg-surface-2 p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="text-right shrink-0">
                  <p className="text-sm font-semibold text-accent">{(c.totalTipAmount ?? 0).toFixed(4)}</p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-text-primary">
                      Tip <span className="text-accent">{rule.tipAmount} ETH</span> when watch &ge; <span className="text-orange-400">{rule.minWatchPercent}%</span>
                    </p>
            ...
            return (
              <div key={pool.id} className="rounded-xl border border-border bg-surface-2 p-4 space-y-3">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-sm font-semibold text-accent">{(pool.currentAmount ?? 0).toFixed(4)} / {pool.goalAmount} ETH</p>
                ...
                <div className="w-full h-2.5 bg-surface-3 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-700 ${
                      pct >= 100
                        ? 'bg-gradient-to-r from-green-400 to-orange-500 shadow-[0_0_8px_rgba(0,217,255,0.4)]'
        ...
        <div className="space-y-2">
          {leaderboard.map((entry) => (
            <div key={entry.creatorId} className="flex items-center gap-3 p-3 rounded-lg border border-border bg-surface-2">
              ...
              <div className="text-right shrink-0">
                <p className="text-sm font-semibold text-accent">{(entry.totalAmount ?? 0).toFixed(4)}</p>
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="rounded-xl border border-neutral-800 bg-neutral-900/60 p-4 sm:p-6">
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-orange-900/50 to-orange-800/30 border border-orange-500/20 flex items-center justify-center">
            <Tv className="w-5 h-5 text-orange-400" />
          </div>
          <div>
            <h2 className="text-base font-semibold text-white">Rumble Integration</h2>
            <p className="text-xs text-neutral-500">AI-powered creator tipping for Rumble&apos;s ecosystem</p>
          </div>
        </div>

        {/* Sub-tabs */}
        <div className="flex gap-1 p-1 rounded-lg bg-neutral-900/60 border border-neutral-800/50 mb-4 overflow-x-auto">
          {tabs.map((tab) => {
            const Icon = tab.icon;
            return (
              <button
