          ...
          <div className="grid grid-cols-4 gap-2">
            {[
              ...
              <div key={s.label} className="text-center p-2 rounded-lg bg-surface-2 animate-list-item-in" style={{ animationDelay: `${i * 50}ms` }}>
                <s.icon className="w-3 h-3 mx-auto mb-1 text-orange-400" />
            ...
            <div className="space-y-1.5">
              ...
              {platform.tokenDistribution.map((t) => (
                <div key={t.token} className="flex items-center gap-2 text-xs">
                  ...
                  <div className="flex-1 h-2 rounded-full bg-surface-3 overflow-hidden">
                    <div className="h-full rounded-full bg-orange-500 transition-all" style={{ width: `${t.percentage}%` }} />
                  </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-3 py-1.5 rounded-lg bg-surface-2 border border-border text-text-primary text-xs focus:border-orange-500 focus:outline-none"
            onKeyDown={(e) => e.key === 'Enter' && searchCreator()}
          />
          <button onClick={searchCreator} disabled={searching || !creatorAddress.trim()} className="px-3 py-1.5 rounded-lg bg-orange-500/10 text-orange-400 text-xs font-medium hover:bg-orange-500/20 disabled:opacity-50 transition-colors btn-press" aria-label="Search creator">
            <Search className="w-3.5 h-3.5" />
          </button>
        </div>

        {creator && (
          <div className="p-3 rounded-lg border border-orange-500/20 bg-orange-500/5 space-y-3 animate-fade-in-up">
                ...
                <div className="flex items-end gap-px h-8">
                  {(() => {
                    ...
                    return creator.dailyIncome.map((d, i) => (
                      <div key={i} className="flex-1 bg-orange-500/60 rounded-t-sm hover:bg-orange-400 transition-colors" style={{ height: `${(d.amount / max) * 100}%`, minHeight: '2px' }} title={`${d.date}: $${d.amount.toFixed(4)}`} />
                    ));
                  })()}
                </div>
              </div>
