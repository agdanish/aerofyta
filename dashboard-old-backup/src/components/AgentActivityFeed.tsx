...
const TYPE_CONFIG: Record<string, { icon: typeof Zap; color: string; label: string }> = {
  ...
  fee_monitor: { icon: TrendingDown, color: 'text-orange-400', label: 'Fees' },
  ...
  memory_learn: { icon: Brain, color: 'text-amber-400', label: 'Memory' },
  schedule_exec: { icon: Clock, color: 'text-lime-400', label: 'Scheduled' },
};
...
export function AgentActivityFeed() {
  ...
  useEffect(() => {
    const connect = () => {
      ...
      es.onmessage = (msg) => {
        ...
        } catch { /* SSE message parse failed — skip malformed event */ }
      };
  ...
  const fetchRecent = useCallback(async () => {
    ...
    } catch { /* activity API not yet connected — keep current list */ }
  }, []);
