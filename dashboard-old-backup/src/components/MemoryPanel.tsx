...
export function MemoryPanel() {
  ...
  const handleStore = async () => {
    ...
    try {
      ...
      setStoreValue('');
      await load();
    } catch { /* store failed — button re-enables */ }
    setStoring(false);
  };
  ...
  const forget = async (id: string) => {
    try { await api.memoryForget(id); await load(); } catch { /* forget failed — no UI change */ }
  };
    ...
    <div className="rounded-xl border border-border bg-surface-1 p-5 space-y-4">
      ...
      <div className="flex gap-1.5">
        <div className="relative flex-1">
          <Search className="absolute left-2 top-1/2 -tranneutral-y-1/2 w-3 h-3 text-text-secondary" />
          <input
            type="text"
            value={searchQuery}
