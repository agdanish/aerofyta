...
export function TipPropagationPanel() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Propagation data initializing');
    }
    setLoading(false);
  };
  ...
  return (
    <div className="rounded-xl border border-border bg-surface-1 p-5 space-y-4">
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {[
            ['Waves', stats.totalWaves, <Waves key="w" className="w-3 h-3 text-orange-400" />],
      ...
      {stats && stats.poolBalance > 0 && (
        <div className="p-2 rounded-lg bg-orange-500/5 border border-orange-500/10 text-xs text-text-secondary">
