            ...
            <>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto">
                  {routes.slice(0, 8).map((route, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-2 p-2 rounded-lg bg-surface-2 border border-border text-xs cursor-pointer hover:border-orange-500/30 transition-colors"
                      onClick={() => {
                    ...
                    >
                      ...
                      <ArrowRightLeft className="w-3 h-3 text-orange-400 shrink-0" />
              ...
              <div className="border-t border-border pt-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 rounded-lg bg-surface-2 border border-border text-sm text-text-primary focus:outline-none focus:border-orange-500/50"
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      min="0"
                      step="0.01"
                      className="w-full px-3 py-2 rounded-lg bg-surface-2 border border-border text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-orange-500/50"
                    />
                  </div>
                </div>
                ...
                <div className="flex gap-2 mt-3">
                  ...
                  <button
                    ...
                    className="px-4 py-2 rounded-lg bg-orange-600 hover:bg-orange-500 text-white text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-1.5"
                  >
                ...
                {executing && (
                  <div className="mt-3 p-3 rounded-lg bg-orange-500/5 border border-orange-500/20 animate-fade-in">
                    <p className="text-xs font-semibold text-orange-400 uppercase tracking-wider mb-2">Bridge Progress</p>
                    <div className="flex items-center gap-2">
                      {['Submitting', 'Bridging', 'Confirming'].map((step, i) => (
                        <div key={step} className="flex items-center flex-1 gap-1">
                          <div className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold ${
                            i === 0 ? 'bg-orange-500/20 text-orange-400 animate-step-pulse' : 'bg-surface-3 text-text-muted'
                          }`}>{i + 1}</div>
                          <span className={`text-xs ${i === 0 ? 'text-orange-400' : 'text-text-muted'}`}>{step}</span>
              ...
              {quote && (
                <div className="rounded-lg bg-orange-500/5 border border-orange-500/20 p-3">
                  <h4 className="text-xs font-medium text-orange-400 mb-2">Bridge Quote</h4>
