...
const FAQ_DATA: FaqItem[] = [
  {
    question: 'What is AeroFyta?',
    answer:
      'AeroFyta is an AI-powered multi-chain payment agent built on the Tether Wallet Development Kit (WDK). It lets you send tips across Ethereum (Sepolia testnet) and TON (Testnet) with intelligent chain selection, NLP commands, and fee optimization.',
  ...
  },
  {
    ...
    answer:
      'AeroFyta currently supports Ethereum (Sepolia testnet) and TON (Testnet). The agent automatically selects the optimal chain based on fees, speed, and the recipient address format.',
  ...
  },
  {
    ...
    answer:
      'AeroFyta includes a Telegram bot integration that lets you send tips directly from Telegram using the /tip command. Configure the bot token in your environment variables and use commands like "/tip 0.01 ETH to 0xABC...".',
  ...
  },
  {
    ...
    answer:
      'Yes. AeroFyta uses a self-custodial architecture powered by Tether WDK. Your private keys are generated and stored locally on your device and never leave it. No keys or seed phrases are sent to any server. You can also backup and restore your wallet.',
    tags: ['security', 'wallet', 'keys', 'custody'],
  },
  {
    question: 'What is WDK?',
    answer:
      'WDK (Wallet Development Kit) is an open-source toolkit by Tether for building wallet applications. It provides secure key management, multi-chain transaction signing, and account abstraction support. AeroFyta is built entirely on WDK.',
      ...
      {open && (
        <div className="mt-3 rounded-xl border border-border bg-surface-1 p-4 sm:p-6 space-y-4 animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -tranneutral-y-1/2 w-4 h-4 text-text-muted pointer-events-none" />
            <input
              type="text"
              value={search}
              onChange={(e) => {
            ...
            {search && (
              <button
                ...
                className="absolute right-3 top-1/2 -tranneutral-y-1/2 text-text-muted hover:text-text-primary transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
            )}
