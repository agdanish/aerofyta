  ...
  return (
    <div className="rounded-xl border border-border bg-surface-1 p-4 sm:p-5">
      ...
      <p className="text-xs text-text-secondary mb-4">
        Click "Run" to execute pre-built demo scenarios that showcase AeroFyta features with real testnet transactions.
      </p>
              ...
              <div key={b.chainId} className="flex items-center justify-between text-xs">
                ...
                <div className="text-right">
                  <span className="font-mono text-text-primary">
                    {(parseFloat(b.nativeBalance) || 0).toFixed(4)} {b.nativeCurrency}
                  </span>
                  {(parseFloat(b.usdtBalance) || 0) > 0 && (
                    <span className="ml-2 text-text-muted">
                      + {(parseFloat(b.usdtBalance) || 0).toFixed(2)} USDT
                    </span>
                  )}
                </div>
              </div>
