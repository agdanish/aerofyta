...
const WELCOME_MESSAGE: ChatMessage = {
  ...
  role: 'agent',
  content:
    "Hey! I'm AeroFyta, your AI payment assistant. I can send tips, check balances, compare fees, and more. Try saying \"help\" to see everything I can do!",
...
function TypingIndicator() {
  return (
    <div className="flex items-start gap-2 mb-3">
      <div className="w-7 h-7 rounded-full bg-gradient-to-br from-blue-500 to-amber-600 flex items-center justify-center text-white text-xs font-bold shrink-0">
        T
      </div>
    ...
    <>
      ...
      {!isOpen && (
        <button
          ...
          className="fixed bottom-5 right-20 z-50 w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-amber-600 text-white shadow-lg hover:shadow-xl hover:scale-105 transition-all flex items-center justify-center"
          title="Chat with AeroFyta Agent"
        >
          <ChatIcon />
          {/* Notification dot */}
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-border bg-surface-2 shrink-0">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-amber-600 flex items-center justify-center text-white text-sm font-bold">
              T
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-sm font-semibold text-text-primary">AeroFyta Agent</div>
              ...
              <div key={msg.id} className={`flex items-start gap-2 mb-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                ...
                <div
                  className={`max-w-[80%] text-sm leading-relaxed ${
                    msg.role === 'user'
                      ? 'bg-gradient-to-br from-blue-500 to-amber-600 text-white rounded-2xl rounded-tr-sm px-4 py-2.5 shadow-sm'
          ...
          <div className="px-3 py-3 border-t border-border bg-surface-2 shrink-0">
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="w-9 h-9 rounded-xl bg-gradient-to-br from-blue-500 to-amber-600 text-white flex items-center justify-center shrink-0 hover:opacity-90 transition-opacity disabled:opacity-40 disabled:cursor-not-allowed"
                title="Send message"
              >
                <SendIcon />
              </button>
