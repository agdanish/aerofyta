...
const EVENT_COLORS: Record<ActivityEventType, string> = {
  ...
  fee_optimized: 'text-orange-400',
  nlp_parsed: 'text-purple-400',
  contact_saved: 'text-orange-400',
  batch_started: 'text-amber-400',
...
const EVENT_BG: Record<ActivityEventType, string> = {
  ...
  fee_optimized: 'bg-orange-500/10',
  nlp_parsed: 'bg-purple-500/10',
  contact_saved: 'bg-orange-500/10',
  batch_started: 'bg-amber-500/10',
        ...
        {events.length === 0 ? (
          ...
          <div className="divide-y divide-border/50">
            {events.map((event, i) => {
              ...
              const isDemo = event.message.startsWith('[Demo]');
              const displayMessage = isDemo ? event.message.replace('[Demo] ', '') : event.message;
              ...
              return (
                <div
                  ...
                  } ${isDemo ? 'opacity-60' : ''} hover:bg-surface-2/50`}
                >
                  {/* Icon */}
                  <div
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-text-primary leading-relaxed">
                      {isDemo && <span className="text-[10px] font-medium text-text-muted bg-surface-2 px-1 py-0.5 rounded mr-1.5">DEMO</span>}
                      {displayMessage}
                    </p>
                    {event.detail && (
