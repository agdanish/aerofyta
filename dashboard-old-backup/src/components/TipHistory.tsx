...
export function TipHistory({ history, loading }: TipHistoryProps) {
  ...
  const handleShare = async (entry: TipHistoryEntry) => {
    ...
    const lines = [
      ...
      ``,
      `Powered by AeroFyta + Tether WDK`,
    ].filter(Boolean);
      ...
      {history.length > 0 && (
        <div className="space-y-2 mb-3">
          ...
          <div className="relative">
            <Search className="absolute left-2.5 top-1/2 -tranneutral-y-1/2 w-3.5 h-3.5 text-text-muted" />
            <input
              type="text"
              value={search}
            ...
            {search && (
              <button
                onClick={() => setSearch('')}
                className="absolute right-2.5 top-1/2 -tranneutral-y-1/2 text-text-muted hover:text-text-primary transition-colors"
              >
                        ...
                        <div className="flex items-center gap-2 px-2 py-1.5 text-sm">
                          ...
                          <button
                            onClick={async () => {
                              ...
                              } catch { /* clipboard API denied — no feedback */ }
                            }}
