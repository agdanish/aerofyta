...
export const api = {
  ...
  /** Get paginated decision audit trail from autonomous loop */
  getDecisionLog: (page = 1, limit = 20) =>
    fetchJson<{ decisions: Array<Record<string, unknown>>; total: number; page: number }>(`/agent/decisions?page=${page}&limit=${limit}`),
  ...
  escrowRelease: (id: string, opts?: { txHash?: string; trigger?: string }) =>
    fetchJson<Record<string, unknown>>(`/escrow/${id}/release`, { method: 'POST', body: JSON.stringify(opts ?? {}) }),
