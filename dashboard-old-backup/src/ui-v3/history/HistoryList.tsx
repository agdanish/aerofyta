...
export function HistoryList({ history, loading }: HistoryListProps) {
  ...
  const handleExport = async () => {
    setExporting(true);
    try { await api.exportHistory('csv'); } catch (err) { console.error('Export failed:', err); setActionError('Failed to export history'); }
  ...
  const handleReceipt = async (entry: TipHistoryEntry) => {
    ...
    } catch (err) { console.error('Failed to load receipt:', err); setActionError('Receipt loading — ensure agent is running'); }
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-5">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[rgba(136,146,176,0.15)] hover:bg-zinc-700 border border-zinc-700/50 text-sm text-zinc-200 font-medium transition-colors disabled:opacity-50"
        >
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-2 rounded-lg bg-zinc-900 border border-[rgba(136,146,176,0.15)] text-sm text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:border-zinc-600 transition-colors"
          />
          {search && (
            <button onClick={() => setSearch('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-[#8892b0]">
              <X className="w-4 h-4" />
            </button>
          )}
        </div>
        <select
          ...
          className="px-3 py-2 rounded-lg bg-zinc-900 border border-[rgba(136,146,176,0.15)] text-sm text-zinc-200 focus:outline-none focus:border-zinc-600"
        >
      ...
      {/* Action error banner */}
      {actionError && (
        <div className="flex items-center justify-between gap-3 px-4 py-2.5 mb-4 rounded-xl bg-red-500/10 border border-red-500/20 text-sm text-red-400">
          <span>{actionError}</span>
          <button onClick={() => setActionError(null)} className="shrink-0 text-red-400/60 hover:text-red-400 transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-1">
          {filtered.map((entry) => {
            ...
            return (
              <div key={entry.id} className={`rounded-xl transition-colors ${isFailed ? 'border-l-2 border-l-red-500 bg-red-500/3' : 'hover:bg-[rgba(136,146,176,0.15)]/30'}`}>
                {/* Row */}
                <button
                ...
                >
                  {/* Status dot */}
                  <span className={`w-2 h-2 rounded-full shrink-0 ${isFailed ? 'bg-red-500' : 'bg-[#85c742]'}`} />

                  {/* Amount — primary visual weight */}
                  <span className="text-base font-bold text-white tabular-nums w-28 shrink-0">
                    {formatNumber(parseFloat(entry.amount), entry.token === 'usdt' ? 2 : 4)} <span className="text-xs font-medium text-[#8892b0]">{token}</span>
                  </span>

                  {/* Arrow + Recipient */}
                  <span className="text-zinc-600 shrink-0">→</span>
                  <span className="font-mono text-sm text-[#8892b0] truncate min-w-0 flex-1">
                ...
                {isExpanded && (
                  <div className="px-4 pb-4 pt-1 space-y-3 border-t border-[rgba(136,146,176,0.15)]/50 animate-slide-down">
                    {/* AI Reasoning */}
                    {entry.reasoning && (
                      <div className="flex items-start gap-2">
                        <Brain className="w-4 h-4 text-purple-400 mt-0.5 shrink-0" />
                        <p className="text-sm text-[#8892b0]">{entry.reasoning}</p>
                      </div>
                    )}

                    {/* Memo */}
                    {entry.memo && (
                      <p className="text-sm text-[#8892b0] italic pl-6">"{entry.memo}"</p>
                    )}
                    ...
                    <div className="flex items-center gap-2 pl-6 flex-wrap">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 px-2 py-1 rounded text-xs text-[#8892b0] hover:text-white hover:bg-[rgba(136,146,176,0.15)] transition-colors"
                      >
                        {copiedTxId === entry.id ? <><Check className="w-3 h-3 text-[#85c742]" /> Copied</> : <><Copy className="w-3 h-3" /> Copy</>}
                      </button>
                      <button
