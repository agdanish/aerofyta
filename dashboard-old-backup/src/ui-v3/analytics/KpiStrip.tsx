...
export function KpiStrip({ stats }: KpiStripProps) {
  ...
  const kpis = [
    { label: 'Total Tips', value: String(Math.round(totalTips)), icon: TrendingUp, color: 'text-[#85c742]', hint: 'All time' },
    { label: 'Volume', value: `$${formatNumber(totalAmount, 2)}`, icon: DollarSign, color: 'text-blue-400', hint: 'USDT equivalent' },
    { label: 'Success Rate', value: `${Math.round(successRate)}%`, icon: CheckCircle2, color: successRate >= 90 ? 'text-[#85c742]' : 'text-amber-400', hint: successRate >= 90 ? 'Healthy' : 'Needs attention' },
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      {kpis.map(({ label, value, icon: Icon, color, hint }) => (
        <div key={label} className="relative overflow-hidden rounded-xl border border-[rgba(136,146,176,0.15)]/50 bg-zinc-900/40 px-5 py-4">
