  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-4">
        <Sparkles className="w-5 h-5 text-[#85c742]" />
        ...
        {modes.map(({ id, label, icon: Icon, desc }) => (
          <button
            ...
            className={`flex flex-col items-start gap-1 p-3 rounded-xl border transition-all text-left ${
              tipMode === id
                ? 'bg-lime-600/10 border-lime-600/40 text-white'
                : 'bg-zinc-900/40 border-[rgba(136,146,176,0.15)]/50 text-[#8892b0] hover:border-zinc-700 hover:text-zinc-200'
            }`}
          >
      ...
      {tipLinkPrefill && (
        <div className="mb-4 px-4 py-3 rounded-lg bg-cyan-500/8 border border-cyan-500/20 text-sm">
          ...
          <span className="text-[#8892b0] ml-2">{tipLinkPrefill.amount} → {tipLinkPrefill.recipient?.slice(0, 12)}...</span>
        </div>
      )}

      {/* Form */}
      <div className="rounded-2xl border border-[rgba(136,146,176,0.15)]/60 bg-zinc-900/30 p-5">
        {tipMode === 'single' && (
          <TipForm
