...
export function ContextFeed({ agentState, trackedTx, onDismissTracked, scheduledTips, onCancelScheduled }: ContextFeedProps) {
  return (
    <div className="space-y-5">
      ...
      {trackedTx && (
        <div className="rounded-xl border border-lime-600/30 bg-lime-600/5 overflow-hidden">
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          <Radio className="w-4 h-4 text-[#85c742]" />
          <h3 className="text-sm font-semibold text-zinc-200">Agent Pipeline</h3>
        </div>
        <div className="rounded-xl border border-[rgba(136,146,176,0.15)]/60 bg-zinc-900/30 overflow-hidden">
      ...
      <div>
        ...
        <div className="rounded-xl border border-[rgba(136,146,176,0.15)]/60 bg-zinc-900/30 overflow-hidden max-h-64 overflow-y-auto">
          <ActivityFeed />
        </div>
      </div>
          ...
          <div className="space-y-1.5">
            {scheduledTips.slice(0, 4).map((tip) => (
              <div key={tip.id} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors ${
                tip.status === 'scheduled' ? 'bg-amber-500/5 hover:bg-amber-500/10' : tip.status === 'executed' ? 'bg-[#85c742]/5' : 'bg-red-500/5'
              }`}>
                {tip.status === 'scheduled' && <Clock className="w-3 h-3 text-amber-400 shrink-0" />}
                {tip.status === 'executed' && <CheckCircle2 className="w-3 h-3 text-[#85c742] shrink-0" />}
                ...
                <span className="text-[#8892b0] font-mono text-xs truncate flex-1">{tip.recipient.slice(0, 8)}...</span>
