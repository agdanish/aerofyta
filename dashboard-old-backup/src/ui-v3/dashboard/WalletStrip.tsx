...
function WalletPill({ balance }: { balance: WalletBalance }) {
  ...
  return (
    <div className="flex-shrink-0 w-56 rounded-xl border border-[rgba(136,146,176,0.15)] bg-zinc-900/50 p-4 hover:border-zinc-700 transition-colors">
      <div className="flex items-center gap-2 mb-3">
        ...
        <a href={`${explorerBase}${balance.address}`} target="_blank" rel="noopener noreferrer" className="text-zinc-600 hover:text-[#8892b0] transition-colors">
      ...
      <div className="space-y-1">
        ...
        {hasUsdt && <p className="text-sm font-medium text-[#85c742] tabular-nums">{formatNumber(animatedUsdt, 2)} <span className="text-xs">USDT</span></p>}
      </div>
      <button onClick={handleCopy} className="mt-2 flex items-center gap-1 text-xs text-zinc-500 hover:text-[#8892b0] transition-colors">
        {copied ? <Check className="w-3 h-3 text-[#85c742]" /> : <Copy className="w-3 h-3" />}
...
export function WalletStrip({ balances, loading }: WalletStripProps) {
  if (loading) {
    return (
      <div className="mb-6">
        <h2 className="text-sm font-semibold text-[#8892b0] uppercase tracking-wider mb-3">Wallets</h2>
        <div className="flex gap-4 overflow-x-auto pb-2">
          {[1, 2, 3].map(i => (
            <div key={i} className="flex-shrink-0 w-56 rounded-xl border border-[rgba(136,146,176,0.15)] bg-zinc-900/50 p-4">
  ...
  return (
    <div className="mb-6" data-onboarding="wallets">
      <h2 className="text-sm font-semibold text-[#8892b0] uppercase tracking-wider mb-3">Wallets</h2>
      <div className="flex gap-4 overflow-x-auto pb-2 -mx-1 px-1 scrollbar-thin scrollbar-thumb-[rgba(136,146,176,0.15)] scrollbar-track-transparent">
