...
export function AiHero({ agentMode, chainCount }: AiHeroProps) {
  const stats = [
    ...
    { label: 'Chains', value: String(chainCount), icon: Radio, color: 'text-[#85c742]' },
      ...
      <div className="relative px-8 py-8">
        ...
        <div className="flex flex-wrap items-center gap-3 mt-5">
          {stats.map(({ label, value, icon: Icon, color }) => (
            <div key={label} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[rgba(136,146,176,0.15)]/50 border border-zinc-700/40">
